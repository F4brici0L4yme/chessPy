...
def place_pieces(board):
    for col in range(8):
        board[7][col] = board[7][col].under(first_row[col].negative()) #
        board[6][col] = board[6][col].under(pawn.negative())
        
    for col in range(8):
        board[0][col] = board[0][col].under(first_row[col])
        board[1][col] = board[1][col].under(pawn)
